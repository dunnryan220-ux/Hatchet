...
import { fetchGBPData } from '@/lib/integrations/google-business';
...
export async function GET(req: NextRequest) {
  ...
  if (!client.gbpLocationId || !client.gbpRefreshToken) {
    return NextResponse.json({ error: 'GBP not connected', notConnected: true }, { status: 404 });
  }

  try {
    const data = await fetchGBPData(client.gbpLocationId, client.gbpRefreshToken, startDate, endDate);
